...
public class BayControllerTest {
    ...
    void testGetById() throws Exception {
        ...
        //When Then
        MvcResult result = mockMvc.perform(get("/api/v1/Bay/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()).andReturn();

        // Assert
        assertGetBayDtoResponse(result.getResponse().getContentAsString());
    }
    ...
    void testPostBay() throws Exception {
        ...
        //When Then
        MvcResult result = mockMvc.perform(post("/api/v1/Bay/")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
                .content(content.writeValueAsBytes(newBayDto)))
                .andExpect(status().isOk()).andExpect(jsonPath("$.data.id").value(1L)).andReturn();
    ...
    void testPutBay() throws Exception {
        ...
        //When Then
        MvcResult result = mockMvc.perform(put("/api/v1/Bay/")
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON)
    ...
    private void assertGetBayDtoResponse(String responseContent) {

        Assertions.assertTrue(JsonPath.read(responseContent, "$.message") != null);
        Assertions.assertTrue(JsonPath.read(responseContent, "$.success").equals(true));
        Assertions.assertEquals(1, (int) JsonPath.read(responseContent, "$.data.rowNumber"));
        Assertions.assertEquals(1, (int) JsonPath.read(responseContent, "$.data.shelfNumber"));
        Assertions.assertEquals(1, (int) JsonPath.read(responseContent, "$.data.levelNumber"));
        Assertions.assertEquals("CART", JsonPath.read(responseContent, "$.data.type"));
        Assertions.assertEquals(5, (int) JsonPath.read(responseContent, "$.data.holldingPoint"));
        Assertions.assertEquals(2, (int) JsonPath.read(responseContent, "$.data.occupiedHoldingPoint"));
        Assertions.assertEquals("001", JsonPath.read(responseContent, "$.data.warehouseCode"));
        Assertions.assertEquals("Wer1", JsonPath.read(responseContent, "$.data.warehouseName"));
        Assertions.assertEquals("Add", JsonPath.read(responseContent, "$.data.warehouseAddress"));
